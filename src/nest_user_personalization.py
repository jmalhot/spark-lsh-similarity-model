...
def load_user_event_logs(spark):
    ...
    # Anonymous logs
    df_a = spark.read.format("json").load(P_EVENT_LOGS_A).filter(
        col("propertyId").isNotNull() & col("userId").isNotNull())
    # Non-Anonymous logs
    df_na = spark.read.format("json").load(P_EVENT_LOGS_NA).filter(
        col("propertyId").isNotNull() & col("userId").isNotNull())
...
def filter_inactive_properties(df):
    ...
    df_active = spark.sql('''
                           ...
                           ''')
    df_filter_active = df_active.select(
        'user_id', 'item_id', 'country', 'timestamp').distinct()
...
def events_popularity_score(df):
    ...
    df_popularity_sc = df_popularity.withColumn(
        "score", psf.dense_rank().over(cnt_score))
...
def user_item_popularity(df):
    ...
    window = Window.partitionBy(
        df_tmp['user_id']).orderBy(
        df_tmp['count_score'].desc())

    return df_tmp.select(
        'user_id',
        'item_id',
        rank().over(window).alias('score'))
...
def user_personalization(df_similarity, df_active):
    ...
    df_save = df_tmp.select(
        ...
        F.struct(
            ...
            F.col("score"))) .select(
                "user_id",
                "Recommendations") .groupby("user_id").agg(
                    F.collect_list("Recommendations").alias("Recommendations"))
