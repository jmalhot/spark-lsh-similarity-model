...
def similar_properties(my_spark, sqlContext, df_events, mode):
    ...
    log_file = 'similar_properties_' + \
        str(datetime.utcnow().strftime('%m_%d_%Y')) + '.log'
    ...
    if (mode == 'CA'):
        ...
        try:
            ...
            # or its good idea to directly cache a dataframe instead of table
            # like below
            ca_df.cache()
            ...
            logging.warning(
                " CA Similarity Model Training took {} seconds.".format(
                    time.time() - ca_start_time))
        ...
        try:
            ...
            logging.warning(
                " US Similarity Model Training took {} seconds.".format(
                    time.time() - us_start_time))
